...
public class DatabaseQueryBuilderService : IDatabaseQueryBuilderService
{
        ...
        QueryBuilderAdvancedRequest req, CancellationToken ct = default)
    {
        ...
        if (req.Tabelas.Count > 5)
            return new DatabaseQueryBuilderResult("", new(), new(), "M�ximo de 5 tabelas permitidas por consulta");

        var tabelasValidas = await ValidarTabelasAsync(req.Tabelas, ct);
        if (tabelasValidas.Count == 0)
            return new DatabaseQueryBuilderResult("", new(), new(), "Nenhuma tabela v�lida encontrada");

        var sqlBuilder = new System.Text.StringBuilder();
        var joinsUsados = new List<RelationshipDto>();
        // Aliases amig�veis e determin�sticos (ex.: TBCHAMADO -> C, TBCLIENTE -> CL)
        ...
        List<string> tabelas, List<string> colunas, List<RelationshipDto> relacionamentos, CancellationToken ct)
    {
        ...
        // Usar relacionamentos confirmados primeiro, depois poss�veis
        ...
        var joinsUsados = new List<RelationshipDto>();
        var tabelasConectadas = new HashSet<string> { tabelas[0] }; // Come�a da primeira tabela
        ...
        while (fila.Count > 0 && tabelasConectadas.Count < tabelas.Count)
        {
            ...
            // Tamb�m verificar relacionamentos inversos
        ...
        // Se n�o conseguiu conectar todas, tentar relacionamentos poss�veis restantes
        if (tabelasConectadas.Count < tabelas.Count)
        {
            ...
            {
                // Tentar conectar a qualquer tabela j� conectada
    ...
    /// <summary>
    /// Aliases amig�veis e determin�sticos (ex.: TBCHAMADO -> C, TBCLIENTE -> CL, TBCHAMADO_ITENS -> CI).
    /// A mesma entrada sempre gera os mesmos aliases, ent�o chamadas independentes a <see cref="GetAlias"/>
