...
| Implantação | Nova/editar Tarefa | `/implantacao/tarefas/novo`, `/implantacao/tarefas/:id/editar` | IM-06 | cenário documentado |
...
- **Objetivo:** Preencher os dados de execução, responsáveis, coluna e chamado de uma Tarefa.
- **Pré-condições:** operador autenticado; projetos, colunas, operadores e chamados disponíveis.
...
- **Resultado esperado:** o formulário valida título, responsável, data, prioridade e etapa quando há projeto; o motivo é obrigatório quando a tarefa é bloqueada. O Status não é enviado pelo formulário: o backend sincroniza Status e Coluna. Na criação, a navegação vai para a lista; na edição, a tela permanece com aviso de sucesso. O projeto selecionado deve ser persistido na edição.
- **Estado atual/bloqueio:** limitação conhecida: o formulário oferece seleção de Projeto, mas `TarefaAtualizarRequest` não contém `ProjetoId` e `TarefaService.AtualizarAsync` não altera `t.ProjetoId`; trocar o projeto na edição não é salvo. A validação de permissões para atribuição também depende do serviço e não é uniforme para todas as escritas.
...
- **Status/automação:** **cenário documentado**; não há teste automatizado do formulário.
...
- **Status/automação:** **cenário documentado**; não há teste automatizado das ações do drawer ou lote.
...
- **Objetivo:** Comparar a estrutura do banco conectado com um arquivo JSON de tabela ou de banco inteiro.
- **Pré-condições:** operador autenticado; arquivo `.json` válido; para modo tabela, selecionar a tabela JCA.
- **Passos:** 1. Abrir `/database/diferencas`. 2. Escolher `Tabela única` ou `Banco inteiro`. 3. Selecionar o JSON, copiar/usar o script de exportação se necessário e clicar em `Comparar`. 4. Filtrar por severidade, expandir tabelas e exportar CSV. 5. Gerar os scripts de correção (opções `gerarBackup`/`modoEstrito`), revisar por aba, validar (estático), copiar ou exportar `.sql`. 6. Limpar.
- **Resultado esperado:** a tabela única lista críticas e avisos conforme o card ativo e mostra o percentual de match; itens compatíveis (`Ok`) ficam só nos cards de contagem e no %, fora das listagens e do CSV. No banco inteiro, os cards escolhem quais tabelas aparecem, a expansão de cada tabela mostra sempre avisos + críticos (nunca `Ok`) e a numeração das críticas reinicia por tabela (1, 2, 3…); tabelas 100% ok aparecem apenas nos chips/contadores. A UI aceita somente JSON, com limite de 5 MB no modo tabela e 20 MB no modo banco. A geração de scripts devolve criações/alterações/índices e FKs com resumo, impacto e itens em `RevisaoManual`; nenhum SQL é executado e nada é persistido. A exportação CSV é local, exclui itens `Ok` e inclui a coluna `numero` nos dois modos; a comparação não altera o banco.
- **Estado atual/bloqueio:** sem limitação específica identificada nesta inspeção; a comparação depende de o arquivo JSON estar no formato aceito e de a conexão permitir metadados. Itens destrutivos só aparecem como comentário/`RevisaoManual`, conforme `modoEstrito`.
- **Rotas/endpoints/tabelas:** `/database/diferencas`; `GET /api/v1/database/tables`; `POST /api/v1/database/compare-schemas` multipart com `schema`, `tabela` e `arquivo`; `POST /api/v1/database/compare-schemas-bulk` multipart com `arquivo`; `POST /api/v1/database/generate-correction-scripts`, `POST /api/v1/database/generate-correction-scripts-bulk` e `POST /api/v1/database/validate-script` com JSON; leitura dos metadados `sys.*`, sem escrita.
...
- **40** histórias estão como `cenário documentado`.
- **1** história (LB-04) está como `validado em teste` por cobertura unitária existente; isso não representa execução da suíte nesta revisão.
- Itens da seção de regressão/histórico não entram no total.
...
2. Não foram encontrados testes de backend, testes end-to-end ou suítes que cubram Login, Acessos, Agenda, Implantação ou Database. A maior parte das histórias, portanto, permanece `cenário documentado`.
...
5. O filtro Database “Apenas possíveis” compara `possivel` com `possivei`; a opção “Sem projeto”, a edição do projeto da Tarefa e os atalhos do Dashboard não são consumidos de forma confiável.
