...
- **Rotas/endpoints/tabelas:** `/login`; `POST /api/v1/auth/login`, público, com corpo `{ usuario, senha, lembrarAcesso }`; tabelas `tboperador` e `tbrefreshtoken`.
...
- **Rotas/endpoints/tabelas:** `POST /api/v1/auth/refresh` com cookie `cc_refresh`; `POST /api/v1/auth/logout` com `[Authorize]`; `tbrefreshtoken` e `tboperador`. O `GET /api/v1/auth/me` existe no backend, mas não é consumido pelo fluxo atual do frontend.
...
- **Rotas/endpoints/tabelas:** `/`; `GET /api/v1/agenda/eventos?inicio=X&fim=Y`, autenticado; leitura de `tbagenda` e das tabelas de apoio de operador/tipo. A busca da Home é local e não tem endpoint.
...
- **Rotas/endpoints/tabelas:** todas as rotas sob `authGuard`; `POST /api/v1/auth/logout` com `[Authorize]`; `tbrefreshtoken` para revogação. Não há endpoint direto de layout.
...
- **Rotas/endpoints/tabelas:** `/implantacao/dashboard`; `GET /api/v1/implantacao/dashboard?projetoId=N`, `GET /api/v1/implantacao/projetos` e `GET /api/v1/implantacao/tarefas?projetoId=N`, todos `[Authorize]`; `tbprojeto`, `tbtarefa`, `tbtarefaapontamento` e `tbtarefareponsavel`.
...
- **Rotas/endpoints/tabelas:** `/implantacao/projetos`; `GET /api/v1/implantacao/projetos/com-etapas` e `GET /api/v1/agenda/operadores`, autenticados; `tbprojeto`, `tbprojetoetapa`, `tbtipoprojeto` e `tboperador`.
...
- **Rotas/endpoints/tabelas:** `/implantacao/projetos/novo`, `/implantacao/projetos/:id/editar`; consultas de apoio: `GET /api/v1/implantacao/tipos-projeto?apenasAtivos=true`, `GET /api/v1/implantacao/projetos/clientes`, `GET /api/v1/agenda/operadores`, `GET /api/v1/implantacao/projetos/etapas-padrao` e `GET /api/v1/implantacao/projetos/proximo-codigo`; `POST /api/v1/implantacao/projetos` e `PUT /api/v1/implantacao/projetos/{id}`; `tbprojeto`, `tbprojetoetapa`, `tbtipoprojeto`, `tbcliente` e `tboperador`.
...
- **Resultado esperado:** o quadro usa colunas ativas, mostra métricas e preserva o filtro de projeto; ao mover o card, a interface envia a nova coluna/ordem e trata limite de trabalho em progresso (WIP), status, bloqueio e motivo. Tarefa bloqueada (criação ou edição) aparece na coluna BLOQUEADO e, ao desbloquear, volta para a coluna do seu status. A criação na própria interface exige título, responsável e data de entrega; com projeto, exige etapa do projeto. O filtro `Minhas tarefas` deve usar o usuário autenticado.
...
- **Passos:** 1. Abrir `/database/diferencas`. 2. Escolher `Tabela única`, `Banco inteiro` ou `Procedures`. 3. Selecionar o JSON (ou o export das procedures), copiar/usar o script de exportação se necessário e clicar em `Comparar`. 4. Navegar pelas 5 abas do relatório (Tabelas faltantes, Colunas faltantes, Tamanhos divergentes, Índices divergentes ou faltantes, Chaves FK e PK divergentes ou faltantes) e conferir os cards fixos de compatíveis e %; exportar CSV. 5. Gerar os scripts de correção, revisar por aba, validar (estático), copiar ou exportar `.sql`. 6. No modo procedures, conferir os corpos lado a lado, filtrar por status/busca e copiar. 7. Limpar.
- **Resultado esperado:** o relatório tem **5 abas** com badge de contagem (a primeira com contagem > 0 é a inicial; `Tabelas faltantes` só aparece no banco inteiro) e **2 cards fixos sem filtro**: compatíveis (apenas contagem) e match (%). Os botões de aba têm rótulo em destaque (fonte maior). **Cada registro da lista exibe um contador sequencial por aba (1, 2, 3…)**, com cor por severidade, e o CSV traz o mesmo número na coluna `numero`. A aba `Tamanhos divergentes` agrupa diferenças de tipo/tamanho e de nullable; `Chaves FK e PK` inclui a comparação de chave primária, que passou a ser exportada pelo script (`"pk": {"nome","colunas"}` ou `null`) e pelo backend (`SchemaInfoDto.Pk`, categoria `Pk`); JSON antigo sem `pk` pula a comparação de PK. No banco inteiro os itens são listados de forma achatada com tag da tabela e os chips (ok / com diferenças / só no arquivo / só no banco) ficam dentro da aba `Tabelas faltantes`. Itens compatíveis (`Ok`) ficam só nos cards de contagem e no %, fora das listagens e do CSV. A geração segue o **banco JCA como fonte da verdade**: divergências e itens só no JCA não geram script; só criações vindas do arquivo (`CREATE_TABLE`, `ADD_COLUMN`, `CREATE_INDEX`, `ALTER_FK`) são geradas, cada uma com `tabela` e `severidadeOrigem`; o `CREATE TABLE` prefere a PK informada no arquivo (com fallback para índice `PK*`). No modo procedures, a resposta traz chips de status (`Compativel`, `Divergente`, `SomenteBanco`, `SomenteArquivo`), busca e comparação de corpos lado a lado, **sem** gerar SQL. A UI aceita somente JSON, com limite de 5 MB no modo tabela e 20 MB no modo banco e nas procedures. Nenhum SQL é executado e nada é persistido; a exportação CSV é local, exclui itens `Ok` e traz as colunas `aba` e `numero` (numeração por aba igual à da tela) nos dois modos de schema; a comparação não altera o banco.
...
- **Rotas/endpoints/tabelas:** `/agenda`; `GET /api/v1/agenda/eventos?inicio=X&fim=Y&responsavelId=Z&funcaoId=N`, `GET /api/v1/agenda/tipos`, `GET /api/v1/agenda/operadores`, `GET /api/v1/agenda/funcoes`, todos `[Authorize]`; `tbagenda`, `tbtipoevento`, `tbagendaparticipante`, `tboperador`, `tbfuncao`, `tbfuncionario` e `tbprojeto`.
...
- **Rotas/endpoints/tabelas:** `/agenda` + modal; `GET /api/v1/agenda/eventos/{id}`, `POST /api/v1/agenda/eventos`, `POST /api/v1/agenda/eventos/lote`, `PUT /api/v1/agenda/eventos/{id}`, `DELETE /api/v1/agenda/eventos/{id}`; `tbagenda`, `tbagendaparticipante`, `tbtipoevento`, `tbprojeto` e `tboperador`.
...
- **Rotas/endpoints/tabelas:** `/agenda`; `PUT/DELETE /api/v1/agenda/eventos/{id}` e `PATCH /api/v1/agenda/eventos/{id}/mover`, autenticados; `tbagenda`, `tbagendaparticipante`, `tbprojeto`, `tbtipoevento`, `tboperador` e `tbfuncao`.
...
- **Rotas/endpoints/tabelas:** `/admin/cadastros/tipos-projeto`, `/novo`, `/editar/:id`; `GET /api/v1/implantacao/tipos-projeto?apenasAtivos=true`, `GET /api/v1/implantacao/tipos-projeto/{id}`, `POST /api/v1/implantacao/tipos-projeto`, `PUT /api/v1/implantacao/tipos-projeto/{id}`, `DELETE /api/v1/implantacao/tipos-projeto/{id}`; todas exigem `[Authorize]` e as escrita exigem `Administrador`; `tbtipoprojeto`.
...
- **Rotas/endpoints/tabelas:** `/implantacao/kanban?perfil=F`; `GET /api/v1/implantacao/tarefas?perfilId=F&perfilModo=excluir`, além dos endpoints de colunas/projetos/operadores do Kanban; `tbtarefa`, `tbprojeto`, `tbcolunakanban` e `tboperador`.
