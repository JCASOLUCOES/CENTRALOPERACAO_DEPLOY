...
- **Resultado esperado:** a tabela única mostra críticas/avisos/compatíveis e percentual de match; o banco inteiro mostra status por tabela. A UI aceita somente JSON, com limite de 5 MB no modo tabela e 20 MB no modo banco. A geração de scripts devolve criações/alterações/índices e FKs com resumo, impacto e itens em `RevisaoManual`; nenhum SQL é executado e nada é persistido. A exportação CSV é local; a comparação não altera o banco.
