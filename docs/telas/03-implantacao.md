...
|--------|-----------|---------|-----------|
| GET | `/api/v1/implantacao/dashboard` | `DashboardService.obter()` | KPIs agregados (`porEquipe` = stub `Geral`) |
| GET | `/api/v1/implantacao/tarefas` | `TarefasService.listar()` | Tarefas para cálculo client-side da Produtividade (filtro de período por `dataInclusao`; série de conclusões por `dataConclusao`; atraso = `dataEntrega ?? dataPrevisao` passada e status ≠ Concluída/Cancelada) |
...
- IDENTIDADE CLEAN (21/09/2026, confirmado no código) + `app-page-header` (21/09/2026, confirmado no código): header via componente reutilizável `PageHeaderComponent` (`dashboard.component.ts:40-57` — `titulo="Visão Geral"`, `descricao="Acompanhe o progresso geral, métricas e o status consolidado do projeto."`, `icone="bi-bar-chart-fill"`; slot `actions` com seletor de período 7d/30d/tudo + botões CSV/Imprimir); overrides inline antigos removidos; título com `<i class="bi bi-bar-chart-fill">`, sem emoji
- **Produtividade (fusão do Relatório):** sinais `periodoProd` (`'30d'` default)/`linhasProd`/`totalProd`/`concluidasProd`/`taxaProd`/`tempoMedioProd`/`slaProd`/`pessoasProd`/`porPrioridadeProd`/`atrasadasProd`/`evolucaoProd`; métodos `aoMudarPeriodoProd()`/`recalcularProd()`/`exportarCsv()`/`imprimir()`/`renderGraficoProd()`/`diasAtraso()`/`barraPessoa()` (`dashboard.component.ts:710-882`); `FormsModule` importado para o `ngModel` do seletor de período; gráfico de evolução (`#evolucaoChart`, barras Chart.js, `temEvolucao()` para empty state) com `destroy()` antes de recriar; série 7/30 dias por `toDateString()` de `dataConclusao`; sem mudança de backend
...
### O que faz
Formulário create/edit de Tarefa. `create` em `/implantacao/tarefas/novo` (aceita `?projetoId=` para pré-selecionar o projeto, vindo do detalhe do projeto); `edit` em `/implantacao/tarefas/:id/editar` (pré-preenche via `obter()`). Validação client-side (`podeSalvar()`: título ≥ 3, `projetoId` obrigatório, `prioridade` 0–3, `ordem` ≥ 0, motivo obrigatório se `bloqueada`). Após create/atualização, navega para `/implantacao/tarefas/:id/editar`.
...
### Payloads
- **Create** → `POST /api/v1/implantacao/tarefas` (`TarefaCriarRequest`: `projetoId*`, `etapaId?`, `projetoEtapaId?`, `colunaKanbanId?`, `titulo*`, `descricao?`, `responsavelId?`, `criadorId*`, `prioridade`, `ordem`, `dataPrevisao?`, `horasEstimadas?`, `chamadoLegadoId?`).
- **Edit** → `PUT /api/v1/implantacao/tarefas/{id}` (`TarefaAtualizarRequest`: `etapaId?`, `projetoEtapaId?`, `colunaKanbanId?`, `chamadoLegadoId?`, `titulo*`, `descricao?`, `responsavelId?`, `status?`, `prioridade`, `ordem`, `dataPrevisao?`, `dataConclusao?`, `horasEstimadas?`, `horasRealizadas?`, `bloqueada`, `motivoBloqueio?`, `usuarioAlteracao*`).
- **Form expõe:** projeto*, prioridade*, etapa, **etapa do projeto (card)** (`projetoEtapaId`, dropdown das 9 fixas via `GET /projetos/{id}/etapas`, default = `EmAndamento`, `NULL` = "Sem card (só totais)"), coluna Kanban, responsável, status (edit), datas previsão/conclusão, horas estimadas/realizadas, bloqueio + motivo, `chamadoLegadoId` ("Chamado Legado (ID)").
- **Backend:** `TarefaDetalhe.ChamadoLegadoId` (`TRF_ChamadoLegadoId`, FK lógica p/ `tbchamado.CHAMADO_ID`); `TarefaAtualizarRequest.Status` opcional (`Enum.TryParse<StatusTarefa>`; `StatusTarefa`: `Backlog`, `AFazer`, `EmAndamento`, `EmHomologacao`, `Concluida`, `Cancelada`); prioridade validada via `Enum.IsDefined(typeof(PrioridadeTarefa))` (`Baixa = 0`, `Media = 1`, `Alta = 2`, `Urgente = 3`) no create e no update (`400 { mensagem }` quando inválida).
...
**Onde está a funcionalidade agora:** seção "Produtividade" do Dashboard (`dashboard.component.ts` — header com seletor de período 7d/30d/tudo + botões CSV/Imprimir; 4 stats: taxa de conclusão, tempo médio criação→conclusão, % dentro do SLA, atrasadas; grids Conclusões por dia via Chart.js, Produtividade por pessoa, Por prioridade e Top 10 atrasadas com link de edição). Cálculo client-side via `TarefasService.listar({})` (`periodoProd`/`linhasProd`/`recalcularProd()`/`exportarCsv()`/`imprimir()`/`renderGraficoProd()`, `FormsModule` importado). Histórico da implementação original (Corretor #8): relatório de SLA/produtividade calculado no frontend a partir de `GET /implantacao/tarefas` (sem endpoint novo), com exportação CSV e impressão (PDF via impressão), IDENTIDADE CLEAN 21/09/2026.
