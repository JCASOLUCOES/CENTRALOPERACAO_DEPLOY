...
| `ProjetoEtapaService` | Nove cards, checklist, documentos, comentários, retorno e progresso | `AppDbContext`, auditoria, logger | Card precisa existir; concluída exige 100%; retorno aceita ordem 1–8 e apenas para trás | Um GET pode criar cards ausentes; conclui (somente com a anterior concluída), bloqueia em cascata, desbloqueia, reseta, grava histórico e recalcula Projeto |
...
| `TarefaService` | CRUD, Kanban, responsáveis, chamados, apontamentos e histórico | DbContext, auditoria, jornada, cards, logger | Tarefa com Projeto exige `projetoEtapaId` do mesmo Projeto (etapa sem projeto é rejeitada na criação; em tarefa sem projeto a etapa é zerada na atualização) e o Projeto não é trocado pelo `PUT`; enums, coluna, responsáveis e chamados são validados; WIP e motivo de bloqueio (exigido na criação/atualização de tarefa bloqueada e na coluna BLOQUEADO); apontamento > 0; atualização/exclusão de apontamento ficam restritas ao próprio para os JWTs emitidos atualmente | Audita, sincroniza a tarefa com a coluna BLOQUEADO (e de volta ao desbloquear), recalcula jornada/cards e pode criar/atualizar evento de Agenda em colunas especiais |
...
Ao criar um Projeto, o controller chama `ProjetoService` e depois `ProjetoEtapaService.InicializarEtapasPadraoAsync`. Alterações de Tarefas recalculam a jornada e sincronizam cards com tarefas não arquivadas. Cards concluídos podem desbloquear o próximo (`Pendente` ou `Bloqueada` → `EmAndamento`, preservando `DataInicio`); a conclusão automática de card só ocorre quando o anterior está concluído; uma tarefa reaberta reabre o card afetado e as etapas posteriores `Concluida` passam a `Bloqueada` em cascata. Toda tarefa com projeto nasce e permanece ligada a um card do mesmo projeto: a criação sem etapa é recusada, a etapa de outro projeto é recusada e a atualização não move a tarefa entre projetos.
