...
`DbSincronizacaoComponent` executa a comparação em três modos — `Tabela única`, `Banco inteiro` e `Procedures` — e monta o resumo com **5 abas** (`ABAS_RELATORIO`/`abaDeCategoria`): `TABELAS FALTANTES` (somente no modo banco inteiro), `COLUNAS FALTANTES`, `TAMANHOS DIVERGENTES` (agrupa as categorias `Tipo` + `Nullable`), `ÍNDICES DIVERGENTES OU FALTANTES` e `CHAVES FK E PK DIVERGENTES E FALTANTES` (categorias `Fk` + `Pk`). Cada aba tem badge de contagem e a inicial é a primeira com contagem > 0 (`tabelas` no banco / `colunas` na tabela); os botões de aba usam rótulo maior (`font-size: 0.95rem`, padding ampliado). Ao lado das abas ficam **2 cards fixos, sem filtro**: `compatíveis` (apenas contagem — `oks` na tabela única, `tabelasOk` no banco) e `match` (percentual); a severidade vira só cor de item. **Cada registro listado traz um badge numerado sequencial por aba (1..N)**, com cor por severidade (`data-sev`), e o CSV usa a mesma numeração na coluna `numero` (contador por rótulo de aba, gerado em `exportarCsv()`); o antigo `numeroCritico`/`marcarNumeros` foi removido. No modo banco inteiro os itens são achatados por aba com tag da tabela, e os chips (ok / com diferenças / só no arquivo / só no banco) aparecem dentro da aba `TABELAS FALTANTES`; os filtros/expanders antigos por severidade foram removidos (`filtro`, `filtroBulk`, `tabelaExpandida`). Itens `Ok` continuam fora das listagens e do CSV (somente contadores e %). O export CSV client-side traz as colunas `aba` e `numero` (mesmo número sequencial exibido no badge do item, contado por aba; no banco inteiro a iteração é tabela a tabela, na ordem da tela) nos dois modos de schema. O componente também gera o script de exportação JSON (schemas — agora com o campo `pk` em cada tabela, com `FOR XML PATH` compatível com SQL Server 2005+) ou T-SQL de 1 linha por procedure (procedures) para o SSMS; no modo procedures há um script próprio (`scriptSqlProcedures`, com escaping de `\`, aspas, quebras de linha e tabs) e o CSV/ação de scripts fica indisponível. A geração de correção chama `gerarScripts`/`gerarScriptsBulk` **sem** corpo de opções.
