...
- Script: `deploy.ps1` (raiz do repositório, ao lado das pastas `Central-Conhecimento-developer/` e `CCBAckend/`).
- Frontend: `Central-Conhecimento-developer/` (Angular 18 SSR).
- Backend: `CCBAckend/Central_BackEnd/` (ASP.NET Core 8, .NET 8).
...
2. **Executar o script** do diretório raiz (o working directory):
   ...
   ```
   O script:
   - roda `npm run build` em `Central-Conhecimento-developer/`;
   - roda `dotnet publish -c Release` em `CCBAckend/Central_BackEnd/`;
   ...
   - publica o backend em `Suporte_Back` usando `app_offline.htm` para liberar o
     lock do worker e **sem tocar em nenhum `appsettings*.json`** do servidor;
...
| Parâmetro | Default |
|---|---|
| `-Publicar` | `$true` (false = só empacota em `deploy/`) |
| `-ServidorRemoto` | `192.168.2.130` |
| `-UsuarioRemoto` | `JCASRV-SUP` (conta ADM) |
| `-DestinoBackendRel` | `inetpub\wwwroot\Suporte_Back` |
| `-DestinoFrontRel` | `inetpub\wwwroot\Suporte_Front` |
| `-BackupBaseRel` | `Users\JCASRV-SUP\Documents\Backup_IIS` |
...
- O script **nunca sobrescreve nem apaga** os `appsettings*.json` do servidor
  (o `appsettings.json` do repositório tem placeholders).
- A config real (banco/JWT/Google) é guardada só no servidor; foi recuperada do
  histórico do git (commit `05978be`) em 07/08/2026.
