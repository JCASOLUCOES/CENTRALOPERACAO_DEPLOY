...
@Component({
  ...
  <div *ngIf="temResultado()" class="db-scripts">
    <div class="db-scripts__header">
      ...
      <p class="text-muted">
        O <strong>arquivo é o alvo</strong>: os scripts ajustam o banco JCA para ficar igual ao arquivo.
        Ações destrutivas (exclusões, conversões incompatíveis) saem <strong>comentadas</strong> —
        nada é executado pelo sistema. Copie, revise no SSMS e execute por sua conta e risco.
      </p>
    </div>

    <div class="db-scripts__barra">
      <label class="db-scripts__check">
        <input type="checkbox" [ngModel]="gerarBackup()" (ngModelChange)="gerarBackup.set($event)">
        <span><i class="bi bi-database-down"></i> Incluir backup automático</span>
      </label>
      <label class="db-scripts__check">
        <input type="checkbox" [ngModel]="modoEstrito()" (ngModelChange)="modoEstrito.set($event)">
        <span><i class="bi bi-shield-check"></i> Modo estrito (só scripts seguros)</span>
      </label>
      ...
      <div class="db-scripts__lista">
        ...
        <div *ngFor="let sc of abaScripts()" class="db-scripts__card"
          ...
          <div class="db-scripts__card-topo">
            ...
            <span *ngIf="sc.backupSugerido" class="db-scripts__tag">
              <i class="bi bi-database-down"></i> backup sugerido
            </span>
          ...
          <details *ngIf="sc.opcoes && sc.opcoes.length" class="db-scripts__alternativas">
            <summary><i class="bi bi-signpost-split"></i>
              Alternativas ao script ({{ sc.opcoes.length }})</summary>
            <div *ngFor="let alt of sc.opcoes" class="db-scripts__alt">
              <small class="text-muted">{{ alt.descricao }}</small>
              <pre class="db-scripts__pre db-scripts__pre--alt"><code [innerHTML]="realcar(alt.sqlFormatado)"></code></pre>
              <button type="button" class="btn btn-sm btn-outline-secondary"
                      (click)="copiar(alt)">
                <i class="bi" [ngClass]="copiadoId() === alt.id ? 'bi-check-lg' : 'bi-clipboard'"></i>
                {{ copiadoId() === alt.id ? 'Copiado' : 'Copiar alternativa' }}
              </button>
            </div>
          </details>
        </div>
  ...
  styles: [`
    ...
    .db-scripts__check {
      ...
      font-size: 0.85rem; font-weight: 600; color: #334155; cursor: pointer; margin: 0;
    }
    ...
    .db-scripts__tag { font-size: 0.72rem; color: #0369a1; background: #e0f2fe; padding: 0.15rem 0.5rem; border-radius: 999px; }
    ...
    .db-scripts__pre--alt { max-height: 10rem; background: #1e293b; }
    ...
    .db-scripts__alternativas { margin-top: 0.55rem; font-size: 0.82rem; }
    .db-scripts__alternativas summary {
      cursor: pointer; font-weight: 600; color: #1e3a8a; user-select: none;
    }
    .db-scripts__alt {
      margin-top: 0.5rem; padding: 0.5rem 0.6rem;
      background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 0.4rem;
    }
    .db-scripts__alt small { display: block; margin-bottom: 0.35rem; }
    .db-scripts__alt .btn { margin-top: 0.4rem; }
...
export class DbScriptsCorrecaoComponent {
  ...
  readonly gerarBackup = signal(true);
  readonly modoEstrito = signal(false);
  ...
  gerar(): void {
    ...
    const opcoes = { gerarBackup: this.gerarBackup(), modoEstrito: this.modoEstrito() };
    this.gerando.set(true);
    this.erro.set(null);
    this.validacoes.set({});

    const req = r ? this.db.gerarScripts(r, opcoes) : this.db.gerarScriptsBulk(rb!, opcoes);
    req.subscribe({
      next: res => {
  ...
  private montarArquivo(): string | null {
    ...
    const cabecalho = [
      ...
      '-- Direção: o arquivo é o alvo; o banco JCA será ajustado.',
      '-- Ações destrutivas estão comentadas; descomente somente após revisar.',
      `-- Backup automático: ${this.gerarBackup() ? 'SIM' : 'NÃO'} | Modo estrito: ${this.modoEstrito() ? 'SIM' : 'NÃO'}`,
      '-- ============================================================',
      ''
    ].join('\n');
  ...
  rotuloTipo(t: SqlScriptTipo): string {
    switch (t) {
      ...
      case 'ALTER_COLUMN': return 'ALTERAR COLUNA';
      case 'ALTER_TYPE': return 'ALTERAR TIPO';
      case 'DROP_COLUMN': return 'EXCLUIR COLUNA';
      case 'DROP_TABLE': return 'EXCLUIR TABELA';
      case 'CREATE_INDEX': return 'CRIAR ÍNDICE';
      case 'DROP_INDEX': return 'EXCLUIR ÍNDICE';
